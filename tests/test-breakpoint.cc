...
int unapply_count;
static void on_unapply() {
  unapply_count++;
}

static void test_adw_breakpoint_condition_length() {
  ...
  g_assert_true(Adw::BreakpointCondition::create_length(
                  Adw::BreakpointCondition::LengthType::MIN_HEIGHT, 200, Adw::LengthUnit::PT)
                  .to_string() == "min-height: 200pt");

  g_assert_true(Adw::BreakpointCondition::create_length(
                  Adw::BreakpointCondition::LengthType::MIN_HEIGHT, 200.5, Adw::LengthUnit::PT)
                  .to_string() == "min-height: 200.5pt");
}

static void test_adw_breakpoint_condition_ratio() {
  ...
  g_assert_true(Adw::BreakpointCondition::create_ratio(
                  Adw::BreakpointCondition::RatioType::MIN_ASPECT_RATIO, 2, 1)
                  .to_string() == "min-aspect-ratio: 2");
  g_assert_true(Adw::BreakpointCondition::create_ratio(
                  Adw::BreakpointCondition::RatioType::MIN_ASPECT_RATIO, 0, 2)
                  .to_string() == "min-aspect-ratio: 0");
}

static void test_adw_breakpoint_condition_and_or() {
  ...
  g_assert_true(Adw::BreakpointCondition::create_and(condition_1, condition_2).to_string() ==
                "max-width: 400px and min-aspect-ratio: 4/3");

  g_assert_true(Adw::BreakpointCondition::create_or(condition_1, condition_2).to_string() ==
                "max-width: 400px or min-aspect-ratio: 4/3");

  // Nested conditions of the same kind don't get parentheses...
  g_assert_true(Adw::BreakpointCondition::create_and(
                  condition_1, Adw::BreakpointCondition::create_and(condition_2, condition_3))
                  .to_string() ==
                "max-width: 400px and min-aspect-ratio: 4/3 and max-aspect-ratio: 2");

  // ...but nested conditions of a different kind do.
  g_assert_true(Adw::BreakpointCondition::create_and(
                  condition_1, Adw::BreakpointCondition::create_or(condition_2, condition_3))
                  .to_string() ==
                "max-width: 400px and (min-aspect-ratio: 4/3 or max-aspect-ratio: 2)");

  g_assert_true(Adw::BreakpointCondition::create_or(
                  Adw::BreakpointCondition::create_and(condition_1, condition_2), condition_3)
                  .to_string() ==
                "(max-width: 400px and min-aspect-ratio: 4/3) or max-aspect-ratio: 2");
}

static void test_adw_breakpoint_condition_parse() {
  auto check = [](const char* input, const char* expected) {
    g_assert_true(Adw::BreakpointCondition::parse(input).to_string() == expected);
  };
  ...
  check("      max-width        :        400     pt       ", "max-width: 400pt");
  ...
  check("max-width: 100px and max-height: 200px", "max-width: 100px and max-height: 200px");
  check("max-width: 100px or max-height: 200px", "max-width: 100px or max-height: 200px");
  check("(max-width: 100px) and max-height: 200px", "max-width: 100px and max-height: 200px");
  check("max-width: 100px and (max-height: 200px)", "max-width: 100px and max-height: 200px");
  check("(max-width: 100px and max-height: 200px)", "max-width: 100px and max-height: 200px");
...
static void test_adw_breakpoint_condition_property() {
  auto condition = Adw::BreakpointCondition::parse("max-width: 400px");
  auto breakpoint = Adw::Breakpoint::create(condition);

  notified = 0;
  breakpoint->property_condition().signal_changed().connect(sigc::ptr_fun(notify_cb));

  g_assert_true(breakpoint->get_condition().to_string() == condition.to_string());

  auto new_condition = Adw::BreakpointCondition::parse("max-width: 200px");
  breakpoint->set_condition(new_condition);
  g_assert_true(notified == 1);
  g_assert_true(breakpoint->get_condition().to_string() == new_condition.to_string());
}

static void test_adw_breakpoint_add_setter() {
  auto condition = Adw::BreakpointCondition::parse("max-width: 400px");
  auto breakpoint = Adw::Breakpoint::create(condition);
  Gtk::Label label;
...
static void test_adw_breakpoint_signals() {
  auto condition = Adw::BreakpointCondition::parse("max-width: 400px");
  auto breakpoint = Adw::Breakpoint::create(condition);
...
int main(int argc, char* argv[]) {
  ...
  g_test_add_func("/Adwaita/Breakpoint/condition_length", test_adw_breakpoint_condition_length);
  g_test_add_func("/Adwaita/Breakpoint/condition_ratio", test_adw_breakpoint_condition_ratio);
  g_test_add_func("/Adwaita/Breakpoint/condition_and_or", test_adw_breakpoint_condition_and_or);
  g_test_add_func("/Adwaita/Breakpoint/condition_parse", test_adw_breakpoint_condition_parse);
  g_test_add_func("/Adwaita/Breakpoint/condition_property", test_adw_breakpoint_condition_property);
  g_test_add_func("/Adwaita/Breakpoint/add_setter", test_adw_breakpoint_add_setter);
