...
static void test_adw_avatar_show_initials(void) {
...
static void test_adw_avatar_custom_image(void) {
  Adw::Avatar avatar(128, TEST_STRING, true);
  ...
  auto texture = Gdk::MemoryTexture::create(
      1, 1, Gdk::MemoryTexture::Format::R8G8B8A8, bytes, 4);
...
static void test_adw_avatar_draw_to_texture(void) {
  Gtk::Window window;
...
static void test_adw_avatar_default_ctor(void) {
  Adw::Avatar avatar;
  g_assert_true(avatar.get_text() == "");
  g_assert_true(avatar.get_icon_name() == "");
  g_assert_true(!avatar.get_custom_image());
...
static void test_adw_avatar_properties(void) {
  Adw::Avatar avatar(TEST_SIZE, "", true);
...
int main(int argc, char *argv[]) {
  ...
  g_test_add_func("/Adwaita/Avatar/show_initials",
                  test_adw_avatar_show_initials);
  g_test_add_func("/Adwaita/Avatar/custom_image", test_adw_avatar_custom_image);
  g_test_add_func("/Adwaita/Avatar/draw_to_texture",
                  test_adw_avatar_draw_to_texture);
