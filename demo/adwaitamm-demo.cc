...
static void show_about(const Glib::RefPtr<Gtk::Application>& app) {
  std::vector<Glib::ustring> developers{
    "Adrien Plazas",  "Alexander Mikhaylenko", "Andrei Lișiță", "Guido Günther",
    "Julian Sparber", "Manuel Genovés",        "Zander Brown",
  };
...
int main(int argc, char** argv) {
  ...
  auto app =
    Adw::Application::create("org.gnome.Adwaitamm1.Demo", Gio::Application::Flags::NON_UNIQUE);

  app->add_action("inspector", sigc::ptr_fun(show_inspector));
  app->add_action("preferences", sigc::bind(sigc::ptr_fun(show_preferences), app));
