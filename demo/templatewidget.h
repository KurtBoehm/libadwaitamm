...
namespace Gtk {
template<auto P, typename T = decltype(P)>
struct PtrFunToMemFun;
...
public:
  explicit TemplateWidgetSetup(GtkWidgetClass* widget_class_) : widget_class(widget_class_) {}

  void set_resource(const Glib::ustring& resource_name) {
    gtk_widget_class_set_template_from_resource(widget_class, resource_name.c_str());
  }

  void bind_widget(const Glib::ustring& widget_name) {
    gtk_widget_class_bind_template_child_full(widget_class, widget_name.c_str(), false, 0);
  }

  void bind_callback(const Glib::ustring& name, GCallback callback) {
    gtk_widget_class_bind_template_callback_full(widget_class, name.c_str(), callback);
  }
  ...
  GType get_class_type() const {
    return G_TYPE_FROM_CLASS(widget_class);
  }

  GtkWidgetClass* get_class() const {
    return widget_class;
  }
};
...
public:
  ...
  void bind_widget(Widget*& widget, const Glib::ustring& name) {
    widget = dynamic_cast<Widget*>(Glib::wrap(GTK_WIDGET(
      gtk_widget_get_template_child(GTK_WIDGET(instance), G_OBJECT_TYPE(instance), name.c_str()))));
  }
};
...
public:
  ...
  TemplateWidget(GtkWidget* obj)
      : CppBaseObjectClass((typename CppBaseObjectClass::BaseObjectType*)(obj)) {}

  virtual ~TemplateWidget() {
    if (this->gobj()) {
      gtk_widget_dispose_template(GTK_WIDGET(this->gobj()), G_OBJECT_TYPE(this->gobj()));
    }
    this->destroy_();
  }
  ...
  static GType get_type() {
    return type_class_.init().get_type();
  }

  static CppObjectClass* wrap(GObject* obj) {
    return dynamic_cast<CppObjectClass*>(
      Glib::wrap_auto(G_TYPE_CHECK_INSTANCE_CAST(obj, CppObjectClass::get_type(), GObject)));
  }
...
template<typename CppObjectClass, typename CppBaseObjectClass>
class TemplateWidget<CppObjectClass, CppBaseObjectClass>::TemplateWidgetClass : public Glib::Class {
public:
  using Class::Class;
  ...
  void register_derived_type(GType base_type,
    ...
    return register_derived_type(base_type, derived_name, instance_init, nullptr);
  }

  void register_derived_type(GType base_type, const char* derived_name,
                             GInstanceInitFunc instance_init, GTypeModule* module) {
    if (gtype_) {
    ...
    if (G_TYPE_IS_FINAL(base_type)) {
      g_critical("TemplateWidgetClass::register_derived_type(): cannot derive from a "
                 "final type!");
      return;
    }
#endif
  ...
  const Class& init() {
    if (!gtype_) {
      ...
      register_derived_type(CppBaseObjectClass::get_base_type(), CppObjectClass::class_name,
  ...
  static Glib::ObjectBase* wrap_new(GObject* obj) {
    auto* result =
      new CppObjectClass(G_TYPE_CHECK_INSTANCE_CAST(obj, CppObjectClass::get_type(), GtkWidget));
