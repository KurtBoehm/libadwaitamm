...
static char* get_transition_name(AdwEnumListItem* item, gpointer /*user_data*/) {
...
void DemoPageLeaflet::setup_template(Gtk::TemplateWidgetSetup& s) {
  signal_next_page = g_signal_new("next-page", s.get_class_type(), G_SIGNAL_RUN_FIRST, 0, NULL,
                                  NULL, NULL, G_TYPE_NONE, 0);

  s.set_resource("/org/gnome/Adwaitamm1/Demo/ui/pages/leaflet/adw-demo-page-leaflet.ui");
  s.bind_callback("get_transition_name", GCallback(get_transition_name));
  s.bind_callback("next_row_activated_cb",
                  Gtk::ptr_fun_to_mem_fun<&DemoPageLeaflet::next_row_activated_cb>());
}

void DemoPageLeaflet::init_widget(Gtk::TemplateWidgetInit& i) {
  p_transition_type = std::make_unique<Glib::Property<Adw::Leaflet::TransitionType>>(
