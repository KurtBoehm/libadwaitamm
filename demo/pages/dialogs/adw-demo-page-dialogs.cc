...
void DemoPageDialogs::setup_template(Gtk::TemplateWidgetSetup& s) {
  ...
  s.set_resource("/org/gnome/Adwaitamm1/Demo/ui/pages/dialogs/adw-demo-page-dialogs.ui");
  s.install_action("demo.message-dialog",
...
void DemoPageDialogs::demo_message_dialog_cb() {
  ...
  Adw::MessageDialog* dialog =
    new Adw::MessageDialog(parent, _("Save Changes?"),
  ...
  dialog->signal_response().connect([this](const Glib::ustring& response) {
    auto toast = new Adw::Toast(Glib::ustring::sprintf(_("Dialog response: %s"), response));
