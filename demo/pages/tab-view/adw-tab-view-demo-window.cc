...
TabViewDemoWindow* TabViewDemoWindow::create() {
  GObject* obj = g_object_new_with_properties(TabViewDemoWindow::get_type(), 0, nullptr, nullptr);
...
void TabViewDemoWindow::setup_template(Gtk::TemplateWidgetSetup& s) {
  ...
  s.bind_callback("page_detached_cb",
                  Gtk::ptr_fun_to_mem_fun<&TabViewDemoWindow::page_detached_cb>());
  s.bind_callback("setup_menu_cb", Gtk::ptr_fun_to_mem_fun<&TabViewDemoWindow::setup_menu_cb>());
  s.bind_callback("create_window_cb",
                  Gtk::ptr_fun_to_mem_fun<&TabViewDemoWindow::create_window_cb>());
  s.bind_callback("indicator_activated_cb",
                  Gtk::ptr_fun_to_mem_fun<&TabViewDemoWindow::indicator_activated_cb>());
  s.bind_callback("extra_drag_drop_cb",
                  Gtk::ptr_fun_to_mem_fun<&TabViewDemoWindow::extra_drag_drop_cb>());

  // FIXME: Ideally we'd like this wrapped as well.
  gtk_widget_class_add_binding_action(s.get_class(), GDK_KEY_t, GDK_CONTROL_MASK, "win.tab-new",
                                      NULL);
  gtk_widget_class_add_binding_action(s.get_class(), GDK_KEY_n, GDK_CONTROL_MASK, "win.window-new",
                                      NULL);
  gtk_widget_class_add_binding_action(s.get_class(), GDK_KEY_w, GDK_CONTROL_MASK, "tab.close",
                                      NULL);
}
...
void TabViewDemoWindow::init_widget(Gtk::TemplateWidgetInit& i) {
  ...
  tab_action_group->add_action("pin",
                               [this]() { view->set_page_pinned(get_current_page(), true); });
  tab_action_group->add_action("unpin",
                               [this]() { view->set_page_pinned(get_current_page(), false); });
  tab_action_group->add_action("close", [this]() { view->close_page(get_current_page()); });
  tab_action_group->add_action("close-other",
                               [this]() { view->close_other_pages(get_current_page()); });
  tab_action_group->add_action("close-before",
                               [this]() { view->close_pages_before(get_current_page()); });
  tab_action_group->add_action("close-after",
                               [this]() { view->close_pages_after(get_current_page()); });
  tab_action_group->add_action("move-to-new-window", [this]() { tab_move_to_new_window(); });
  auto needs_attention_action = tab_action_group->add_action_bool("needs-attention");
  ...
  loading_action->signal_change_state().connect(
    [this, loading_action](const Glib::VariantBase& base) {
      Glib::Variant<bool> loading = Glib::VariantBase::cast_dynamic<Glib::Variant<bool>>(base);
  ...
  indicator_action->signal_change_state().connect(
    [this, indicator_action](const Glib::VariantBase& base) {
      Glib::Variant<bool> indicator = Glib::VariantBase::cast_dynamic<Glib::Variant<bool>>(base);
      tab_change_indicator(indicator_action, indicator.get());
    });

  auto icon_action = tab_action_group->add_action_bool("icon");
  icon_action->signal_change_state().connect([this, icon_action](const Glib::VariantBase& base) {
    Glib::Variant<bool> enable_icon = Glib::VariantBase::cast_dynamic<Glib::Variant<bool>>(base);
...
void TabViewDemoWindow::init_icon_names(const Glib::RefPtr<Gtk::IconTheme>& theme) {
  if (!icon_names.empty()) {
    return;
  }
...
Glib::RefPtr<Adw::TabPage> TabViewDemoWindow::add_page(const Glib::RefPtr<Adw::TabPage>& parent,
  ...
  Glib::Binding::bind_property_value(content->property_text(), page->property_tooltip(),
                                     Glib::Binding::Flags::SYNC_CREATE,
...
Glib::ustring TabViewDemoWindow::get_indicator_tooltip(const Glib::RefPtr<Adw::TabPage>& page) {
  void* p = page->get_data("adw-tab-view-demo-muted");
  bool muted = (intptr_t)p;
...
void TabViewDemoWindow::tab_change_indicator(const Glib::RefPtr<Gio::SimpleAction>& action,
                                             bool indicator) {
...
void TabViewDemoWindow::tab_change_icon(const Glib::RefPtr<Gio::SimpleAction>& action,
                                        bool enable_icon) {
  Glib::RefPtr<Adw::TabPage> page = get_current_page();
  if (enable_icon) {
    Glib::RefPtr<Gio::Icon> icon = get_random_icon();
...
void TabViewDemoWindow::tab_duplicate() {
  ...
  Glib::RefPtr<Adw::TabPage> page = add_page(parent, parent->get_title(), parent->get_icon());
  ...
  page->set_data("adw-tab-view-demo-muted", parent->get_data("adw-tab-view-demo-muted"));
...
void TabViewDemoWindow::setup_menu_cb(AdwTabPage* page_, AdwTabView* view_) {
  ...
  set_tab_action_enabled("move-to-new-window", !page || (!pinned && n_pages > 1));
...
gboolean TabViewDemoWindow::extra_drag_drop_cb(AdwTabPage* page_, GValue* value) {
  Glib::RefPtr<Adw::TabPage> page = Glib::wrap(page_);
  if (page) {
    page->reference();
  }
...
void TabViewDemoWindow::set_tab_action_enabled(const Glib::ustring& name, bool enabled) {
  Glib::RefPtr<Gio::SimpleAction> action =
    std::dynamic_pointer_cast<Gio::SimpleAction>(tab_action_group->lookup_action(name));
  action->set_enabled(enabled);
}

void TabViewDemoWindow::set_tab_action_state(const Glib::ustring& name, bool state) {
...
bool TabViewDemoWindow::text_to_tooltip(const GValue* input_, GValue* output_) {
  ...
  Glib::ustring tooltip =
    Glib::Markup::escape_text(Glib::ustring::sprintf(_("Elaborate tooltip for <b>%s</b>"), title));
  ...
  Glib::Value<Glib::ustring>& output = reinterpret_cast<decltype(output)&>(*output_);
