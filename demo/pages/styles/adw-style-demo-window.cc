...
static GtkSelectionMode selection_mode_for_folded(gpointer /*data*/, gboolean folded) {
...
StyleDemoWindow* StyleDemoWindow::create() {
  GObject* obj = g_object_new_with_properties(StyleDemoWindow::get_type(), 0, nullptr, nullptr);
...
void StyleDemoWindow::setup_template(Gtk::TemplateWidgetSetup& s) {
  s.set_resource("/org/gnome/Adwaitamm1/Demo/ui/pages/styles/adw-style-demo-window.ui");
  s.bind_widget("header_bar_window");
  s.bind_widget("status_page_window");
  s.bind_widget("sidebar_window");
  s.bind_widget("sidebar_leaflet");

  s.bind_callback("selection_mode_for_folded", GCallback(selection_mode_for_folded));
  s.bind_callback("sidebar_back_cb", Gtk::ptr_fun_to_mem_fun<&StyleDemoWindow::sidebar_back_cb>());
  s.bind_callback("sidebar_forward_cb",
...
void StyleDemoWindow::init_widget(Gtk::TemplateWidgetInit& i) {
  ...
  simple_action_group->add_action(Gio::PropertyAction::create("devel", proxy_devel));
  ...
  simple_action_group->add_action(
    Gio::PropertyAction::create("progress", Glib::PropertyProxy<bool>(this, "progress")));
...
void StyleDemoWindow::header_bar_cb(GtkWidget* sender, const char* /*name*/, GVariant* /*param*/) {
  Adw::StyleDemoWindow* self = Adw::StyleDemoWindow::wrap(G_OBJECT(sender));
  self->header_bar_window->present();
}

void StyleDemoWindow::status_page_cb(GtkWidget* sender, const char* /*name*/, GVariant* /*param*/) {
  Adw::StyleDemoWindow* self = Adw::StyleDemoWindow::wrap(G_OBJECT(sender));
  self->status_page_window->present();
}

void StyleDemoWindow::sidebar_cb(GtkWidget* sender, const char* /*name*/, GVariant* /*param*/) {
  Adw::StyleDemoWindow* self = Adw::StyleDemoWindow::wrap(G_OBJECT(sender));
